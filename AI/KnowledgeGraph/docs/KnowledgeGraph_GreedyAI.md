...
+ 知识图谱的存储：
  + RDF存储系统：以三元组的形式存储。Jena
  + 图数据库：以属性图的形式存储。Neo4j
  + 两者不同点：
    + 对属性的处理：
    + 对同一种类型的关系的处理：
...
+ 知识图谱的构建：
  + 关键原则：
    ...
    + 紧密结合业务场景，清晰反映现有业务逻辑，适应未来可变化的业务逻辑。
  + 常用技术：
    + 命名实体识别(Named Entity Recognition)
    + 关系抽取(Relation Extraction)
    + 实体统一(Entity Resolution)：例如NYC与纽约
    + 指代消解(Coreference Resolution)：It指代什么？
...
+ 证券投资的核心问题：通过各种数据的分析(行情数据、基本面、新闻、论坛等)，来做买卖决策(买什么、什么时候买，卖什么、什么时候卖)。
+ 知识图谱最适合解决的问题：跟复杂关系分析相关的问题
+ 事件分析：
  + 构建证券知识图谱：比如公司、人、概念、行业、产品等
    + 数据源：
      + 上市公司信息
      + 行情数据
      + 研报
      ...
      + 论坛
    + 挑战：非结构化数据的处理
  + 情感分析：时间是正面还是负面？SVM or NN
  + 相关性分析：历史事件怎么影响股价？
    + 挑战：事件稀疏性
+ 利用知识图谱进行证券分析的难点：（在国外，KENSHO公司是这个领域的代表）
  + 通过知识图谱我们可以比较容易看出一个事件有可能对哪些资产有影响，但具体有对大的影响？
  + 事件稀疏性
  + A股市场特点
  + 数据的获取，比如公司的主要产品、供应链等
  + 知识图谱中的推理
  + 怎么把金融理论结合到知识图谱中来

### 4.2 聊天机器人

+ 什么场景下需要用到知识图谱？**需要返回某种实体的就可以用知识图谱**。
  + 比如对于一个证券类聊天机器人
    + “怎么看未来几个月科大讯飞的走势”。开放式问题，根据过去一年的走势预测未来走势
    + “科大讯飞过去一个月最低价是多少”。直接查询
    + ”科大讯飞的股东有哪些“。知识图谱
    + ”科大讯飞的高管中，哪些人又担任其他上市公司的董事会？“。知识图谱
  + 这些问题的特点：需要返回某种类型的实体。
...
+ 个性化教育：
  + 构建完整的知识点体系：知识图谱
  + 课程设计：课程(Instructional)、测评(Assessment)
  + 诊断学生技能
  + 个性化推荐
+ 知识点体系：知识点(节点)
+ 课程设计：课程(节点)、测评(节点)、测评难度系数(属性)
+ 用户技能诊断：建模
+ 个性化推荐：找到最合理的学习路径。用户技能的当前状态->目标状态
+ 知识点图谱怎么构建：
  + 由专家来构建：
  + 半自动构建：
  + 全自动构建：
...
+ 传统推荐系统中一些潜在问题：
  + 数据稀疏性(即冷启动问题)：购买记录的稀疏性(商品千千万，购买寥无几)，新用户没有购买记录。
  + 没有用到其它隐含信息(implicit information)：比如用户的基本信息、社交关系、点击行为等。
+ 推荐知识图谱的优势
  + 包含各类实体的属性(用户属性、商品属性)
  + 用户与商品之间互动的多样性(查看、点击等)
  + 用户与用户之间的关系(社交网络等)
...
+ 人工定义规则，进行不一致性验证。例如：朋友的朋友是不是朋友？
+ 用离散数学生成新的推理规则
  + 基于first order logic的方法论

### 5.2 分布式表示法

+ 优点：可以很好地解决数据稀疏性的问题
+ 经典例子：词向量
+ 核心思想：**图谱中的每个节点/关系用向量表示**。
+ 分布式表示源于自然语言处理，原来的词袋模型(0-1表示法)无法计算单词相似度，而且向量的维度是跟词典的维度一样的；后来发展出了word2vec，一般100、200、300维，极大地降低了模型复杂度。
...
+ TransE(Translating Embedding)算法
  ...
  + 其它挑战：
    + 怎么把节点的属性加到模型里
    + 怎么把二度或更高维度的neighbor信息考虑到模型里
    + 实体可能拥有多种含义，怎么处理这种一对多的情况？
