...
// Constrcutor  默认输出构造  用于在parse阶段，PluginCreator用于创建该插件时调用的构造函数，需要传递权重信息以及参数
DetectionOutput::DetectionOutput(DetectionOutputParameters params)
    : param(params)
{
}

// 参数 输出构造 用于在clone阶段，复制这个plugin时会用到的构造函数
DetectionOutput::DetectionOutput(DetectionOutputParameters params, int C1, int C2, int numPriors)
    : param(params)
    , C1(C1)                // Channel size of the locData tensor numPriors * numLocClasses * 4
    , C2(C2)                // Channel size of the confData tensor numPriors * param.numClasses
    , numPriors(numPriors)  // Number of bounding boxes per sample
{
}

// Parameterized constructor 序列话 输出构造. 用于在deserialize阶段，用于将序列化好的权重和参数传入该plugin并创建
...
Dims DetectionOutput::getOutputDimensions(int index, const Dims* inputs, int nbInputDims)
{
    ...
    if (index == 0)
    {
        return DimsCHW(1, param.keepTopK, 7);  // [image_id, label, confidence, xmin, ymin, xmax, ymax]
    }
...
{
    // Input order {loc, conf, prior} 规定输入的顺序是{loc_data, conf_data, priorbox_data}
...
//-------------- NMSPluginCreator ---------------
// Plugin creator constructor.   //继承BaseCreator，是插件工厂类，用于根据需求创建该插件
...
// Creates the NMS plugin 用于解析构造plugin
IPluginV2Ext* NMSPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc)
{
    const PluginField* fields = fc->fields;
    // Default init values for TF SSD network.   : Single Shot MultiBox Detector

    params.codeType = CodeTypeSSD::TF_CENTER;  
    // 输入在内存中的顺序是 loc_data, priorbox_data, conf_data  
    // 规定输入的顺序是{loc_data, conf_data, priorbox_data}
    ...
    for (int i = 0; i < fc->nbFields; ++i)
    {
        ...
        if (!strcmp(attrName, "shareLocation"))
        {
            ...
            params.shareLocation = static_cast<int>(*(static_cast<const int*>(fields[i].data))); //static_cast 没有运行时类型检查来保证转换的安全性
        }
